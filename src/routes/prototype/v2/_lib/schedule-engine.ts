import type { QuestionnaireAnswers, GeneratedSchedule, SchedulePhase, PrototypeMealItem, EczemaSeverity, ReintroductionDayInfo, PrototypeMeal, DailyAssessment, ReintroductionEvaluation, AllergenOutcome, SkinStatusOutcome, TrainingReminder } from './types';
import { PROTOCOL_SLUGS, REINTRODUCTION_ORDER, addDays, isDateInRange, getCategoryBySlug } from './data';
...
): { meals: PrototypeMeal[]; assessments: DailyAssessment[]; evaluations: ReintroductionEvaluation[]; mutatedSchedule: GeneratedSchedule } {
  const meals: PrototypeMeal[] = [];
