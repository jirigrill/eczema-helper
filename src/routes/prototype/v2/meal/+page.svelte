<script lang="ts">
  ...
  import type { PrototypeState, PrototypeMeal, PrototypeMealItem, AmountSize } from '../_lib/types';
  import { detectConflicts, getEliminatedSlugsForDate, getReintroductionDayInfo } from '../_lib/schedule-engine';
  import { CATEGORIES, AMOUNT_LABELS, MEAL_TYPE_LABELS, MEAL_TYPE_ICONS, getCategoryBySlug, todayIso, addDays, formatDateLongCs } from '../_lib/data';
  ...
  let currentItems = $state<PrototypeMealItem[]>([]);
  let mealLabel = $state('');
  ...
  function isConflictItem(item: PrototypeMealItem): boolean {
...
{#if showSuccess}
  <div class="fixed bottom-28 left-1/2 -translate-x-1/2 bg-success text-white text-sm rounded-xl px-5 py-3 shadow-lg z-50 text-center min-w-[200px]">
    ...
    <a href="/prototype/v2/day" class="block text-white/90 text-xs mt-1.5 underline underline-offset-2">
