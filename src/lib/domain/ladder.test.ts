...
function makeLadder(allergenId: ProtocolAllergenId, steps: LadderStep[]): Ladder {
  return { allergenId, steps };
}
...
const eggsLadder: Ladder = makeLadder('eggs', [
  { id: 'rung-1', anchor: 'portion',  isEvaluationCheckpoint: false },
  { id: 'rung-2', anchor: 'portion',  isEvaluationCheckpoint: false },
  { id: 'rung-3', anchor: 'package',  isEvaluationCheckpoint: true  },
]);
...
describe('currentRung', () => {
  it('returns null when the meal history has no matching allergen items', () => {
    const meals: Meal[] = [];
    expect(currentRung('eggs', meals, eggsLadder)).toBeNull();
  });
  ...
  it('returns the first rung when only the first-rung anchor has been logged', () => {
    ...
    expect(currentRung('eggs', meals, eggsLadder)?.id).toBe('rung-1');
  });
  ...
  it('advances to the second rung once a second matching anchor is logged on a later meal', () => {
    ...
    expect(currentRung('eggs', meals, eggsLadder)?.id).toBe('rung-2');
  });
  ...
  it('reaches the top rung when the final anchor is logged after the earlier anchors', () => {
    ...
    const rung = currentRung('eggs', meals, eggsLadder);
  ...
  it('preserves the highest rung reached even when a smaller dose is logged afterwards', () => {
    ...
    expect(currentRung('eggs', meals, eggsLadder)?.id).toBe('rung-3');
  });
});
...
describe('nextLegalStep', () => {
  it('returns the first step when the current rung is null', () => {
    expect(nextLegalStep(null, eggsLadder)?.id).toBe('rung-1');
  });

  it('returns the next single step above the current rung', () => {
    expect(nextLegalStep(eggsLadder.steps[0], eggsLadder)?.id).toBe('rung-2');
    expect(nextLegalStep(eggsLadder.steps[1], eggsLadder)?.id).toBe('rung-3');
  });

  it('returns null once the top of the ladder is reached', () => {
    const top = eggsLadder.steps[eggsLadder.steps.length - 1];
    expect(nextLegalStep(top, eggsLadder)).toBeNull();
  });
  ...
  it('cannot express a multi-step advance — the return is a single step or null', () => {
    ...
    const returned = nextLegalStep(eggsLadder.steps[0], eggsLadder);
    const idx = eggsLadder.steps.findIndex((s) => s.id === returned?.id);
    expect(idx).toBe(1);
  });
});
...
describe('ALLERGENS ladders', () => {
  it('every protocol allergen carries a ladder', () => {
    const withProtocol = ALLERGENS.filter(
      (a): a is typeof a & { protocol: object } => 'protocol' in a && a.protocol !== undefined
    );
    for (const allergen of withProtocol) {
      expect((allergen as { ladder?: unknown }).ladder, `missing ladder on ${allergen.id}`).toBeDefined();
    }
  });

  it('each ladder rung inherits isEvaluationCheckpoint from the legacy ProtocolDay.isEvaluationDay at the same index', () => {
    for (const allergen of ALLERGENS) {
      const rec = allergen as {
        id: string;
        protocol?: { days: readonly { isEvaluationDay: boolean }[] };
        ladder?: { steps: readonly { isEvaluationCheckpoint: boolean }[] };
      };
      if (!rec.protocol || !rec.ladder) continue;
      expect(
        rec.ladder.steps.length,
        `rung count mismatch on ${rec.id}`
      ).toBe(rec.protocol.days.length);
      for (let i = 0; i < rec.protocol.days.length; i++) {
        expect(
          rec.ladder.steps[i].isEvaluationCheckpoint,
