...
export type LadderOverrideRepository = {
  ...
  /** Return every stored override in any order. */
  listAll(): Promise<Result<Ladder[], string>>;
  /** Delete the override for `allergenId`. No-op (Ok) when nothing is stored. */
  remove(allergenId: string): Promise<Result<void, string>>;
};
