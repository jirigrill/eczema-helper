...
function makePhoto(id: string, observationId: string, region: SkinPhoto['region'] = 'face'): SkinPhoto {
	return {
		id,
		observationId,
		region,
...
describe('createSkinPhotoSession join', () => {
	...
	it('emits multiple photos when one observation has many photos', async () => {
		...
		expect(rows.map((r) => r.id).sort()).toEqual(expect.arrayContaining(['photo-many-1', 'photo-many-2']));
	});
	...
	it('emits photos from multiple observations for the same date', async () => {
		...
		expect(rows.map((r) => r.id).sort()).toEqual(expect.arrayContaining(['photo-multi-obs-A', 'photo-multi-obs-B']));
	});
