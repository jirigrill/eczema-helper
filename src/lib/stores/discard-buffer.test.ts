...
describe('discardBuffer store', () => {
  ...
  it('writeBuffer stores a WorkingMeal snapshot', () => {
    ...
    writeBuffer({ kind: 'compose', workingMeal: meal, mealType: 'lunch', date: '2026-01-01', returnTo: '/day/2026-01-01' });
  ...
  it('clearBuffer resets to null', () => {
    writeBuffer({ kind: 'compose', workingMeal: mealWithConfirmed(), mealType: 'lunch', date: '2026-01-01', returnTo: '/day/2026-01-01' });
    clearBuffer();
    expect(get(discardBuffer)).toBeNull();
  });

  it('stores mealType and returnTo alongside the working meal', () => {
    writeBuffer({ kind: 'compose', workingMeal: mealWithConfirmed(), mealType: 'breakfast', date: '2026-06-12', returnTo: '/day/2026-06-12' });
    const buf = get(discardBuffer);
  ...
  it('subsequent writeBuffer overwrites the previous snapshot', () => {
    writeBuffer({ kind: 'compose', workingMeal: mealWithConfirmed(), mealType: 'lunch', date: '2026-01-01', returnTo: '/a' });
    const fresh = mealWithConfirmed();
    writeBuffer({ kind: 'compose', workingMeal: fresh, mealType: 'dinner', date: '2026-01-02', returnTo: '/b' });
    const buf = get(discardBuffer);
  ...
  it('round-trips kind="edit" — back-out of a dirty edit', () => {
    writeBuffer({ kind: 'edit', workingMeal: mealWithConfirmed(), mealType: 'lunch', date: '2026-01-01', returnTo: '/day/2026-01-01' });
    expect(get(discardBuffer)?.kind).toBe('edit');
  });

  it('round-trips kind="delete" — explicit delete with undo', () => {
    writeBuffer({ kind: 'delete', workingMeal: mealWithConfirmed(), mealType: 'lunch', date: '2026-01-01', returnTo: '/day/2026-01-01' });
