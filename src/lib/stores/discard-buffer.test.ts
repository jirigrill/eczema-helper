...
describe('discardBuffer store', () => {
  ...
  it('writeBuffer stores a WorkingMeal snapshot', () => {
    ...
    writeBuffer({ kind: 'meal-compose', workingMeal: meal, mealType: 'lunch', date: '2026-01-01', returnTo: '/day/2026-01-01' });
    const buf = get(discardBuffer);
    expect(buf).not.toBeNull();
    if (buf && (buf.kind === 'meal-compose' || buf.kind === 'meal-edit' || buf.kind === 'meal-delete')) {
  ...
  it('clearBuffer resets to null', () => {
    writeBuffer({ kind: 'meal-compose', workingMeal: mealWithConfirmed(), mealType: 'lunch', date: '2026-01-01', returnTo: '/day/2026-01-01' });
    clearBuffer();
    expect(get(discardBuffer)).toBeNull();
  });

  it('stores mealType and returnTo alongside the working meal', () => {
    writeBuffer({ kind: 'meal-compose', workingMeal: mealWithConfirmed(), mealType: 'breakfast', date: '2026-06-12', returnTo: '/day/2026-06-12' });
    const buf = get(discardBuffer);
  ...
  it('subsequent writeBuffer overwrites the previous snapshot', () => {
    writeBuffer({ kind: 'meal-compose', workingMeal: mealWithConfirmed(), mealType: 'lunch', date: '2026-01-01', returnTo: '/a' });
    const fresh = mealWithConfirmed();
    writeBuffer({ kind: 'meal-compose', workingMeal: fresh, mealType: 'dinner', date: '2026-01-02', returnTo: '/b' });
    const buf = get(discardBuffer);
    if (buf && buf.kind === 'meal-compose') {
  ...
  it('round-trips kind="meal-edit" — back-out of a dirty meal edit', () => {
    writeBuffer({ kind: 'meal-edit', workingMeal: mealWithConfirmed(), mealType: 'lunch', date: '2026-01-01', returnTo: '/day/2026-01-01' });
    expect(get(discardBuffer)?.kind).toBe('meal-edit');
  });

  it('round-trips kind="meal-delete" — explicit meal delete with undo', () => {
    writeBuffer({ kind: 'meal-delete', workingMeal: mealWithConfirmed(), mealType: 'lunch', date: '2026-01-01', returnTo: '/day/2026-01-01' });
