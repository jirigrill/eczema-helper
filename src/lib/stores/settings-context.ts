...
/**
 * liveQuery shell over the `settings` singleton. Emits `null` until the row is
 * seeded (first-run completion) and re-emits on every change, so consumers see
 * the live feedingStage master switch (#567).
 */
