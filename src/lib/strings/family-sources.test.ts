...
describe('familySources axes', () => {
  it('fruit has tuzemské · bobuloviny · exotické (in render order)', () => {
    const keys = (familySources as Record<string, readonly { key: string }[]>).fruit?.map((s) => s.key);
    expect(keys).toEqual(['tuzemske', 'bobuloviny', 'exoticke']);
  });
...
describe('source-axis integrity', () => {
  it('every Food.sourceGroup exists in its family axis', () => {
    ...
    for (const food of FOODS as readonly FoodLite[]) {
      ...
      expect(axis, `food '${food.id}' has sourceGroup but family '${food.familyId}' has no axis`).toBeDefined();
