...
- **Feeding stage — a live master switch.** Ladders are keyed by `FeedingStage` (`breastfed | mixed | solids`). The stage is a user-controlled setting, seeded at onboarding and changeable in Settings, held in the `settings` Dexie singleton (`SettingsData.feedingStage`) and read live via `settingsContext` (#567). Consumers thread it in as a parameter (`buildScheduleContext(..., feedingStage)`, `rungAtDayInPhase(..., feedingStage)`); there is no hardcoded stage constant. Defaults to `breastfed` at onboarding since v1 tracks a breastfed newborn (ADR-0001), but the `mixed`/`solids` ladders are now reachable by switching the setting.
