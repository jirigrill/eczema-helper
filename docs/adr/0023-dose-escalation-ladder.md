...
- **Feeding stage — a live master switch.** Ladders are keyed by `FeedingStage` (`breastfed | mixed | solids`). The stage is a user-controlled setting, picked at onboarding and changeable in Settings, held in the `settings` Dexie singleton (`SettingsData.feedingStage`) and read live via `settingsContext` (#567). Consumers thread it in as a parameter (`buildScheduleContext(..., feedingStage)`, `rungAtDayInPhase(..., feedingStage)`); there is no hardcoded stage constant and no fallback — a ready schedule always has a seeded settings row (both written in one onboarding transaction), so consumers hold at `loading` until `settingsContext` emits rather than inventing a value. The onboarding picker pre-selects `breastfed` since v1 tracks a breastfed newborn (ADR-0001), but the `mixed`/`solids` ladders are reachable by picking another stage there or switching later in Settings.
