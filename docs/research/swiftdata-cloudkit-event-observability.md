...
**Update, 2026-08-17 (later the same day): the ceiling was cleared and the remaining questions are now measured on a physical device.** A signing identity was issued (`Apple Development: Jiri Grill (57V7L4QL47)`, team `A4MJZ465YY`) once an iPhone was registered, and the probe ran against real iCloud on an **iPhone 15 Pro, iOS 26.5.2**, signed into a personal Apple ID, container `iCloud.jirigrill.eczema`, `CKAccountStatus = available`. See [#713 item 5 measured](#713-item-5-one-logical-initial-sync-emits-exactly-one-import-event) below, which also closes the three gaps this section listed as unreachable. The sections above remain as written — they were accurate for what was reachable at the time.
...
1. **Which `CKError` codes actually arrive in `Event.error`.** No terminal event was ever produced, so no error was observed. `quotaExceeded` / `notAuthenticated` reaching the app remains **unverified** — exactly as the ticket framed it. Apple types the field only as `(any Error)?`. *(Still unverified after the on-device runs: every event succeeded, so the field stayed `nil`. See [item 5](#713-item-5-one-logical-initial-sync-emits-exactly-one-import-event).)*
2. **Whether an observer registered *after* the container still receives events.** Probed directly (`probe_late.swift`, registering at t=0.5s); the process was already dead. **Unanswered, and it matters**: a real app builds its container in `@main`/`init` and typically subscribes from a view or service created later. If setup events are spent by then, the first and most diagnostic event is missed. Until measured, the spec should require registration **before** the `ModelContainer` is created. *(**Now answered on device — a late observer does receive events, including `.setup`.** The rule survives as a precaution, not a requirement; see [item 5](#the-three-704-gaps-now-closed-by-the-same-runs).)*
3. **Whether one logical initial sync emits one `.import` event or many** — #713 item 5. One `.setup` event was seen; no `.import` ever ran, so this was **not measurable at the time**, and Apple never states a count. Two Apple staff answers also reframe what such an event can mean — see below. *(**Now measured on device: exactly one `.import`, at both 100 and 4,100 rows**, with rows arriving incrementally inside it. See [item 5](#713-item-5-one-logical-initial-sync-emits-exactly-one-import-event).)*
4. ~~**Whether the notification is posted on a background queue**~~ — **closed from documentation**, see above: the CloudKit private serial queue. Serial, so not concurrent; not the main thread, so a `@MainActor` hop is required. *(Confirmed by measurement: background threads only, peak concurrency 1.)*

**Where these answers came from.** Items 1–2 were blocked on a provisioning profile when this section was written; item 2 is now measured on device and item 1 remains open for want of a failing account state. Item 3 was pursued as a documentation question after the entitlement ceiling made it unmeasurable, and is now measured. Item 4 is answered outright. The searches behind items 3–4 were exhaustive of Apple's public surface: reference docs carry abstracts only, with **zero** Discussion prose on any event symbol, so the substantive material exists solely in session transcripts, sample code, forum answers and the SDK headers.
...
## #713 item 5: one logical initial sync emits exactly one `.import` event

**Measured, 2026-08-17, on a physical device.** iPhone 15 Pro, iOS 26.5.2, Xcode 27.0 (27A5237l), Swift 6 language mode, signed with `Apple Development: Jiri Grill (57V7L4QL47)` (team `A4MJZ465YY`) and the team provisioning profile authorising `iCloud.jirigrill.eczema` for Development and Production. Real iCloud account signed in; `CKAccountStatus = available` in every run.

**Method.** Two phases against one CloudKit zone. Phase A inserts N rows into a fresh store and waits for the export. The app is then **uninstalled** — which discards the local store entirely — reinstalled, and phase B launches against the same populated zone, so what it performs is a genuine from-scratch initial import. The count that answers the ticket is the number of distinct `Event.identifier` values of type `.import`, not the number of notifications: each event is notified twice, once in flight (`endDate == nil`) and once terminal.

**The answer: one.** At both fixture sizes tested:

| Zone contents | Distinct `.import` events | Import duration | Row arrival |
|---|---|---|---|
| 100 rows | **1** | 1.2s → 2.9s (1.7s) | one jump, 0 → 100 |
| 4,100 rows | **1** | 1.2s → 15.7s (14.5s) | **incremental**, ~200-row steps |

The 41× larger zone did **not** produce more `.import` events — it produced one longer event. So the events are per logical sync, not per batch, and a "first import finished" heuristic is coherent in the narrow sense that a single terminal `.import` does bound the initial catch-up.

**But the row count climbs incrementally inside that one event**, which is the operationally important half of the finding. In the 4,100-row run the local store went 0 → 200 → 400 → … → 4,100 in ~200-row steps over 14.5s, all under one in-flight `.import` id, with the terminal notification arriving only at the end. A UI observing events alone sees nothing between start and finish while the store grows underneath it. Anything that needs to reflect partial progress must observe the store (SwiftData queries), not the event stream; the event tells you only that a catch-up is in flight or done.

This does not disturb the [no-sync-complete-signal finding](#there-is-no-sync-is-complete-signal-and-an-import-event-is-not-one) above. One `.import` bounds *one* catch-up operation; per Apple's DTS answer it still says nothing about whole-store convergence, and new server-side changes can arrive while the event is being handled. **The count being 1 does not make "fully synced" derivable** — #687's indicator stays scoped to *last successful import at T* / *in flight* / *last failed with E*.

### The three #704 gaps, now closed by the same runs

1. **Delivery thread and concurrency.** Every callback arrived on a **background** thread — never the main thread — across four distinct `NSThread` instances in one run (numbers 4, 5, 6, 8, 10, 11 across runs). **Peak concurrent callbacks was 1** in every run, confirming from measurement what the iOS 27 SDK header said about a private serial queue: events are serialised, so an observer needs no internal locking, but it does need a `@MainActor` hop before touching UI.
2. **Which `CKError` codes reach `Event.error`.** Still **unobserved** — every event in every run succeeded (`errors reaching Event.error: none`). Provoking `quotaExceeded` / `notAuthenticated` needs a deliberately failing account state, which these runs did not create. Recorded as measured-absent rather than answered: the field stayed `nil` throughout a healthy sync.
3. **Whether a late observer still sees events.** **It does.** An observer registered *after* `ModelContainer` init received 8–12 events, including the `.setup` event that fires ~0.1s after init — in the 100-row phase B it saw the same `setup` id as the early observer, both the in-flight and terminal notification. So the "register before the container" rule in the spec is a **precaution, not a requirement**: `NotificationCenter` delivery does not depend on registration preceding container construction. Keep the rule anyway — the margin is ~40ms and an app doing real work at init could plausibly miss it — but it is no longer justified by an observed loss.

**Probe caveat worth recording.** The probe as written for the simulator blocks the main thread with `RunLoop.run` and draws no UI; on a real device the iOS watchdog killed it with signal 9 at ~6.7s. It had to be restructured as a proper app target with the probe body on a detached thread and a trivial `WindowGroup` to stay alive. Any future on-device probe needs the same shape.

**On the simulator.** Even with the real identity and profile, `simctl launch` was still refused (`SBMainWorkspace` denial), and a control build carrying **no** iCloud entitlements at all was refused identically — so that particular wall is about ad-hoc-signed hand-assembled bundles, not the restricted entitlements. The physical device was the shorter path once a certificate existed. This does not contradict [the ceiling section](#the-ceiling-is-the-signing-identity-not-the-simulator) — the simulator's iCloud stack is real and an iCloud account signs in fine (verified: `iCloud` and `CloudKit` rows in the device's `Accounts3.sqlite`) — it only means a properly signed *app target* is required there too, not a `swiftc` + `codesign` bundle.
