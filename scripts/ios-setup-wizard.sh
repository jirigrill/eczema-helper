...
say ""
say "  Enrol as an ${BOLD}individual${RESET}. The owner ruled out an s.r.o. for v1, having"
say "  been shown the 5.1.1(ix) and insurance arguments below. This is settled,"
say "  not provisional - do not stop to reconsider it here."
say ""
warn "Known and accepted tradeoffs of that choice:"
...
info "rejection risk at submission, not at enrolment - so it surfaces late, and"
info "the fallback if it bites is to form an entity and re-enrol then."
info ""
info "The insurance research (issue #681) also leaned toward an s.r.o., because"
info "the one Czech product that covers software as a product worldwide is shaped"
info "for companies. Consequence of enrolling as an individual: personal"
info "liability is not shielded by an entity, so the insurance question becomes"
info "the mitigation rather than the corporate form. Revisit before shipping,"
info "not before the App Review query."
say ""
...
say "    Now more load-bearing than #681 assumed: enrolling as an individual"
say "    means no entity stands between you and a claim, so cover is the only"
say "    mitigation. #681 also found exactly one Czech product covering software"
say "    as a product worldwide, and no insurer publishes eligibility for"
say "    individuals - so confirm you can actually be covered before shipping."
