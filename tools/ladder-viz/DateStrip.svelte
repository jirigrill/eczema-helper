<!-- Date scrubber — pure calendar navigation over the selected run's own days.
     Move forward/back with the arrows or ←/→. -->
<script lang="ts">
  let { days, selected = $bindable() }: { days: readonly string[]; selected: string } = $props();
