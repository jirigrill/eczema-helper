...
> Source: nav-redesign.html `<style>` block + Tailwind extension; see `src/app.css` for runtime token names.
...
- **Reintro Accent** (`{colors.reintro-accent}`): #4A7C6F teal — exclusive to the Test phase of the elimination protocol (not used in nav-redesign, but reserved by the system).
...
- **Mono** — System mono stack: `ui-monospace, SFMono-Regular, "SF Mono", Menlo`. Used **only** on the document chrome (frame tags `▸ DNES`, version eyebrows `_design / nav-v1.7`). Never inside the simulated app screens.
...
| `{typography.variant-label}` | 12px | 400 | 1.30 | 0.4px (mono) | Document version eyebrow (`_design / nav-v1.7`) |
...
- This is the **only** place in the system where a long-running phase appears as inline chrome. Everywhere else, the phase is full-card content (in the program-redesign Section).
...
When pulling a new screen into this system (e.g., aligning the program-redesign), verify:
